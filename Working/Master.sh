...
then
    ...
    then
        ...
        cd /usr/local
        sparkCores=`jq '.sparkCores' info.json`
        echo "export SPARK_WORKER_MEMORY=$sparkCores" | sed "s/['\"]//g" >> /usr/local/kockpit-tools/spark-3.1.3-bin-hadoop3.2/conf/spark-env.sh
...
then
    ...
    while read i;
    do
        echo "ssh-copy-id -i /home/$user/.ssh/id_rsa.pub $user@$i" | sed "s/['\"]//g" >> SSH.sh
    done
    /bin/bash SSH.sh
    sudo rm -rf SSH.sh
...
then
    ...
    then
        ...
        #change according to the python version
